...
class listener implements EventSubscriberInterface
{
	...
	public function add_page_vars($event)
	{
		...
		$urls = [
			...
			'stationRemoveUrl'	=> $this->safe_route('salvocortesiano_radioglobe_station_remove', [], false),
		...
		$player_config = $urls + [
			...
			'listenRepeat'		=> ($toast && !empty($this->config['radioglobe_toast_repeat'])) ? 60 * max(1, min(1440, (int) $this->config['radioglobe_toast_repeat_minutes'])) : 0,
			'coverSpin'			=> !empty($this->config['radioglobe_cover_spin']) ? max(5, min(86400, (int) $this->config['radioglobe_cover_spin_every'])) : 0,
			'coverSpinStyle'	=> (isset($this->config['radioglobe_cover_spin_style']) && $this->config['radioglobe_cover_spin_style'] === 'flat') ? 'flat' : 'flip',
			'commentsEnabled'	=> !empty($this->config['radioglobe_comments_enabled']),
			'canRemoveStation'	=> !$guest && $this->auth->acl_get('m_radioglobe_stations'),
			'nowPlaying'		=> !empty($this->config['radioglobe_nowplaying']),
			'resumeOnLoad'		=> !isset($this->config['radioglobe_player_resume']) || !empty($this->config['radioglobe_player_resume']),
			'isRadioPage'		=> $is_page,
	...
	protected function js_lang()
	{
		$keys = [
			...
			'locateDenied'		=> 'RADIOGLOBE_LOCATE_DENIED',
			'locateFailed'		=> 'RADIOGLOBE_LOCATE_FAILED',
			'locateAccuracy'	=> 'RADIOGLOBE_LOCATE_ACCURACY',
			'locatePoor'		=> 'RADIOGLOBE_LOCATE_POOR',
			'locateSave'		=> 'RADIOGLOBE_LOCATE_SAVE',
			'locateSavedOk'		=> 'RADIOGLOBE_LOCATE_SAVED_OK',
			'locateSavedInUse'	=> 'RADIOGLOBE_LOCATE_SAVED_IN_USE',
			'locateAgain'		=> 'RADIOGLOBE_LOCATE_AGAIN',
			'locateUsePlace'	=> 'RADIOGLOBE_LOCATE_USE_PLACE',
			'locateWrong'		=> 'RADIOGLOBE_LOCATE_WRONG',
			'locateWrongHelp'	=> 'RADIOGLOBE_LOCATE_WRONG_HELP',
			'removeStation'		=> 'RADIOGLOBE_REMOVE_STATION',
			'removeStationAsk'	=> 'RADIOGLOBE_REMOVE_STATION_ASK',
			'removedStation'	=> 'RADIOGLOBE_REMOVED_STATION',
